...
function inconnuboyLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [ABDULLAH] ${new Date().toISOString()}: ${message}`);
}
...
async function inconnuboyPair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `ABDULLAH_lock_${sanitizedNumber}`;
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'ABDULLAH' };
            }
        });
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        ...
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© ABDULLAH' } }
                    }},
        ...
        inconnuboyLog(`AbdullahPair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'ABDULLAH is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 ABDULLAH — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        ...
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { ABDULLAHLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
            if (!activeSockets.has(number)) {
                const mockRes = { headersSent: false, json: () => {}, status: () => mockRes };
                await ABDULLaHPAIR(number, mockRes);
                await delay(2000);
            }
        }
