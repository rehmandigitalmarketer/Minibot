...
const inconnuboyConfigSchema = new mongoose.Schema({
    ...
    config: {
        ...
        AUTO_STATUS_MSG: { type: String, default: 'Hello from ABDULLAH! 🔥' },
...
async function saveSessionToMongoDB(number, credentials) {
    try {
        ...
        console.log(`📁 [ABDULLAH] Session saved for ${cleanNumber}`);
        return true;
    } catch (error) {
    ...
    try {
        ...
        if (config) {
            ...
            const defaultConfig = {
                ...
                AUTO_STATUS_MSG: 'Hello from ABDULLAH! 🔥',
                AUTO_LIKE_EMOJI: ['❤️', '👍', '😮', '😎'],
                ANTIDELETE: 'false'
            };
...
async function updateUserConfigInMongoDB(number, newConfig) {
    try {
        ...
        console.log(`⚙️ [ABDULLAH] Config updated for ${cleanNumber}`);
        return true;
    } catch (error) {
